...
#### Simplification: Unifying OS logLevel with CocoaLumberJack logLevel
   ...
   For example, if we set the CocoaLumberJzack logLevel to `debug`, but the OS logLevel is `default` then the app would produce `debug` and `info` messages but they would all immediately be thrown away by the unified system log, without being stored anywhere. (Update: Actually, `info` would get stored to memory first, and get written to disk on an `error`j or `fault` messages. See the WWDC Talk [6]) If the situation was reversed, then the unified system log would be expecting `debug` and `info` messages but the app would never produce them.
